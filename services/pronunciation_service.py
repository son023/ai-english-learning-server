...
class PronunciationService:
    ...
    def evaluate_pronunciation(
        self, original_text: str, transcribed_text: str, confidence: float = 1.0
    ) -> Tuple[PronunciationScore, List[WordError], float]:
        """
    ...
    def text_to_phonetic(self, text: str) -> str:
        ...
        for pattern, phonetic in sorted(
            self.phonetic_map.items(), key=len, reverse=True
        ):
    ...
    def get_word_errors(self, original: str, transcribed: str) -> List[WordError]:
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "replace":
                for k, (orig_word, trans_word) in enumerate(
                    zip(orig_words[i1:i2], trans_words[j1:j2])
                ):
    ...
    def calculate_scores(
        self, wer_score: float, confidence: float, error_count: int
    ) -> PronunciationScore:
        """Calculate pronunciation scores. Lưu ý: intonation sẽ được cập nhật từ prosody service ở pipeline."""
        pronunciation = max(0, (1 - wer_score) * 100)
        fluency = 0
        intonation = 0  # Sẽ được cập nhật ở pipeline
        stress = 0
    ...
    def get_feedback(
        self, scores: PronunciationScore, word_errors: List[WordError]
    ) -> str:
